(function (global, factory) {
    ...
    var MoveLine = function MoveLine(map, userOptions) {
        ...
        Marker.prototype.draw = function (context) {
            ...
            context.fillText(this.city, pixel.x, pixel.y - 20);
            context.restore();
        };
        ...
        MarkLine.prototype.getPointList = function (from, to) {
            ...
            //不让飞线挡在覆盖物前方
            let fromLen = this.from.color == "yellow" ? 6 : 10;
            if (points.length > (fromLen * 2 + 4)) {
                points.length = points.length - fromLen - 2;
                points.splice(0, fromLen);
            }
            return points;
        };
        ...
        MarkLine.prototype.drawLinePath = function (context) {
            ...
            context.strokeStyle = this.from.hasOwnProperty("color") ? this.from.color : options.colors[this.id];
        ...
        var init = function init(map, options) {
            ...
            CanvasLayers.push(baseLayer);
            CanvasLayers.push(animationLayer);
